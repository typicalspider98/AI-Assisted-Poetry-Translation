...
def parse_entries(content):
    ...
    while i < len(lines):
        ...
        if line and (current_word is None or (i > 0 and lines[i - 1].strip() == "" and not line.startswith("Date:"))):
            if current_word and (meaning or examples or sources):  # 存储上一个单词的定义（即使 Definition 为空，也存储例句）
                definitions.append({
            ...
            if current_word and definitions:
                entries.append({"word": current_word, "definitions": definitions})
...
def convert_txt_to_json(txt_file, json_file):
    """
    读取 txt 词典文件，并转换为 JSON 格式，支持多个词的识别
    """
...
# 调用转换函数
convert_txt_to_json("a_words.txt", "test2.json")