    ...
    while next_tag and next_tag.name == "p":
        ...
        if blockquote:
            for p in blockquote.find_all("p"):  # 遍历所有 <p>，获取多个例句
                text = ' '.join([t.get_text(strip=True) if hasattr(t, 'get_text') else t for t in p.contents])
...
# 保存到 JSON
with open("nz_dictionary_A4.json", "w", encoding="utf-8") as f:
    json.dump(entries, f, ensure_ascii=False, indent=4)

print("✅ A 页面爬取完成，数据已保存为 nz_dictionary_A4.json！")
