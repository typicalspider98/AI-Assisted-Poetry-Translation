...
def generate_instruction_text(user_query: str) -> str:
    ...
    instruction = (
        ...
        "生成的中文prompt要求：\n"
        "1.prompt中首先指出用户提供的中文诗歌原内容，并提出将所给中文诗歌翻译成英文诗歌这个要求。\n"
        "2.prompt应参考用户给出的中文诗歌的具体内容，选择恰当的语言描述本首诗歌在翻译过程中应注意的事项。"
        "（例如：尽量传达原诗的核心思想、情感和主旨，避免曲解或过度延伸；"
        "保留诗歌中的文化意象，并选择合适的英文表达；语言要有韵味，保持诗性表达，如节奏、押韵（若能做到）、简练、修辞等；"
        "对一些特有的文化典故、历史人物或风俗需做注解、意译或文化转化，避免读者误解；根据具体诗句灵活取舍，重意境时意译，重结构或修辞时偏直译）\n"
        "3.prompt应指出在翻译时要体现新西兰地区英语的语言风格和文化特点。"
        "（关于什么是新西兰英语，我指的是下面可能会提供给你的一部分关键词，这些关键词取自由一本新西兰英语词典生成的向量数据库。"
        "在翻译本诗的过程中，有些英文单词在新西兰英语中有特殊的表达，我们将这些特殊的使用情况从向量数据库中取出来，"
        "以关键词及例句的形式展示，你在生成prompt时可以参考这些词的使用场景，看是否可以将其融入你的诗歌翻译中。"
        "要尽可能保留诗歌的原有含义及意境，同时兼具新西兰英语的表达特色。）"
    )
