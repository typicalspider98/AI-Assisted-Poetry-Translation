...
MODEL_ID = "deepseek-ai/DeepSeek-R1-Llama-8B"
...
def generate_boss_prompt(user_query: str) -> str:
    ...
    这里我们调整了生成策略参数：
      - max_new_tokens: 最大生成 token 数，设为150
      - min_length: 最小生成 token 数，设为100，确保输出较为充实
      - do_sample: 启用采样策略，避免生成过于保守
      - temperature: 温度参数设为0.7，平衡随机性和确定性
      - top_p: nucleus 采样，设为0.9，保证生成多样性
    """
    instruction = (
    ...
    outputs = boss_pipeline(
        instruction,
        max_new_tokens=150,  # 最大生成 token 数上限
        min_length=100,  # 至少生成100个 token，避免内容过短
        do_sample=True,  # 启用采样生成，增加多样性
        temperature=0.7,  # 温度控制生成随机性
        top_p=0.9  # nucleus 采样，选择累计概率达到90%的 token 集合
    )
...
def review_translation_with_boss(prompt_context: str, candidate_translation: str) -> str:
    ...
    outputs = boss_pipeline(
        instruction,
        max_new_tokens=200,  # 审核反馈可能需要更多 token 数
        min_length=100,  # 至少生成100个 token 的反馈
        do_sample=True,
        temperature=0.7,
        top_p=0.9
    )
