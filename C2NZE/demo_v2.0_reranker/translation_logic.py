...
def generate_instruction_text(user_query: str) -> str:
    ...
    instruction = (
        ...
        f"3.prompt应指出在翻译时要体现新西兰地区英语的语言风格和文化特点，但你在创建prompt时务必不要给出具体用词建议，我怕你存在这方面的幻觉。"
    ...
    instruction = (
        ...
        f"诗歌内容如下：\n{user_query}\n\n"
        f"输出内容如下：\n"
        f"- System Prompt: 告诉指导对象要扮演什么身份\n"
        f"- Poem: 诗歌内容\n"
        f"- Prompt: 指导翻译的建议列表\n\n"
        f"要求如下：\n"
        f"1. 直接给出中文回答。\n"
        f"2. prompt应参考用户给出的中文诗歌的具体内容，选择恰当的语言描述本首诗歌在翻译过程中应注意的事项，例如：\n"
        f"    - 尽量传达原诗的核心思想、情感和主旨，避免曲解或过度延伸；保留诗歌中的文化意象，并选择合适的英文表达；\n"
        f"    - 语言要有韵味，保持诗性表达，如节奏、押韵（若能做到）、简练、修辞等；\n"
        f"    - 对一些特有的文化典故、历史人物或风俗需做注解、意译或文化转化，避免读者误解；\n"
        f"    - 根据具体诗句灵活取舍，重意境时意译，重结构或修辞时偏直译\n"
        f"3. Prompt 应指出翻译过程中应体现新西兰地区英语的语言风格与文化特点。但在当前生成Prompt阶段，请勿直接给出具体的新西兰英语词汇或句子，以避免因缺乏上下文产生语言幻觉。"
        f"4. 请注意：本系统在生成提示词之后，可能会提供一组与本诗相关的新西兰英语关键词及其释义（由新西兰本地词典构建的向量数据库检索而得）。这些词语将在后续阶段展示并注入提示词中，以增强本地化表达。故在回答Prompt 时，仅需为这类后续信息预留合理的语言结构，而不必自行设想词汇内容。\n"
    )
