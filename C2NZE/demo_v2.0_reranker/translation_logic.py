...
def generate_instruction_text(user_query: str) -> str:
    ...
    instruction = (
        ...
        f"对一些特有的文化典故、历史人物或风俗需做注解、意译或文化转化，避免读者误解；根据具体诗句灵活取舍，重意境时意译，重结构或修辞时偏直译）\n"
        f"3. Prompt 应指出翻译过程中应体现新西兰地区英语的语言风格与文化特点。但在本阶段生成提示词时，请勿直接给出具体的新西兰英语词汇，以避免因缺乏上下文产生语言幻觉。"
        f"此外，请注意：本系统在生成提示词之后，可能会提供一组与本诗相关的新西兰英语关键词及其释义（由新西兰本地词典构建的向量数据库检索而得）。这些词语将在后续阶段展示并注入提示词中，以增强本地化表达。"
        f"在生成初始 prompt 时，你仅需为这类后续信息预留合理的语言结构，而不必自行设想词汇内容。\n\n"
    )
